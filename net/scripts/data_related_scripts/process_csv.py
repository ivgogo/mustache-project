...
def main(data_dir, out_dir):
    ...
    for df, sub_dir in zip((a,b),('train', 'val')):
        df[df.columns[-1]] = df[df.columns[-1]].mask(df[df.columns[-1]].isin((0,1)), 0)
        df[df.columns[-1]] = df[df.columns[-1]].mask(df[df.columns[-1]].isin((3,4,5)), 1)
        df[df.columns[-1]] = df[df.columns[-1]].mask(df[df.columns[-1]].isin((2,9)), 3)
        df[df.columns[-1]] = df[df.columns[-1]].mask(df[df.columns[-1]].isin((6,7,8)), 2)

        print('\nAfter merging',sub_dir,':')
        print(sorted(dict(df[df.columns[-1]].value_counts()).items()))
        print('\nWriting to disk...')

        x = df[df.columns[:184]]; dx = df[df.columns[184:368]]; d2x = df[df.columns[368:552]]
        y = df[df.columns[-1]]
